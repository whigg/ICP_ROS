...
void ScanMatcherNode::process_scan_match(const sensor_msgs::LaserScan::ConstPtr& input_scan) {
  ...
  if (has_previous_scan_) {
    icp_.run_scan_matcher(current_scan, previous_scan_);
    Eigen::Matrix4d transform = icp_.get_transformation();
    int last_iters = icp_.get_last_iterations();

    std::cout<<"Transform found after "<<last_iters<< " iterations: \n \n "<< transform<<std::endl;
    sensor_msgs::PointCloud2 curr_scan_pc = convert_output_cloud(icp_.get_transformed_pointcloud().transpose());
    publish_transform(prev_scan_pc_, curr_scan_pc);
    previous_scan_ = icp_.get_transformed_pointcloud().transpose();
